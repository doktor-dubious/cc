...
[
    ...
    // ════════════════════════════════════════════════════════════════════════════
    {
      ...
      safeguards: [
        ...
        {
          id: '13.6', title: 'Deploy a Host-Based Intrusion Detection Solution',
          definition: 'Deploy a host-based intrusion detection or EDR solution on enterprise endpoints and servers to detect malicious activity at the host level.',
          purpose: 'Detect attacks that operate within a host (process injection, lateral movement, credential theft).',
          why: 'Host-level visibility catches attacks invisible to network sensors, especially with encryption and modern fileless techniques.',
          ...ig(null,
            { scope: 'Standard EDR / HIDS deployment.', approach: 'Deploy EDR agents on servers and workstations with central console.', example: 'A mid-sized company deploys Microsoft Defender for Endpoint or SentinelOne across all endpoints.', resources: 'Moderate — EDR platform.' },
            { scope: 'Enterprise XDR with hunting.', approach: 'Deploy XDR with proactive threat hunting and integration with SIEM.', example: 'An enterprise uses CrowdStrike Falcon or Palo Alto Cortex XDR with 24/7 SOC.', resources: 'Significant — XDR platform + SOC.' }),
        },
        {
          id: '13.7', title: 'Deploy a Host-Based Intrusion Prevention Solution',
          definition: 'Deploy a host-based intrusion prevention solution (HIPS) capable of blocking malicious activity in real time on enterprise endpoints and servers.',
          purpose: 'Stop attacks at the host before they cause damage.',
          why: 'Detection alone is insufficient — blocking malicious activity reduces incident impact and dwell time.',
          ...ig(null, null,
            { scope: 'Enterprise host-based prevention.', approach: 'Enable preventive mode in EDR/XDR with automated response policies.', example: 'An enterprise enables block-mode in CrowdStrike with automated network isolation on critical alerts.', resources: 'Significant — mature EDR deployment + tested policies.' }),
        },
        {
          id: '13.8', title: 'Manage Access Control for Remote Assets',
          definition: 'Manage access control for remote enterprise assets including conditional access policies based on device posture, identity, and risk signals.',
          purpose: 'Ensure only compliant and trusted devices/identities can access enterprise resources remotely.',
          why: 'Remote access expands the attack surface — perimeter authentication alone is insufficient.',
          ...ig(null,
            { scope: 'Conditional access policies.', approach: 'Require MFA, managed-device check, and location-based access policies.', example: 'A mid-sized company uses Microsoft Entra Conditional Access requiring compliant devices for cloud apps.', resources: 'Moderate — identity provider with conditional access.' },
            { scope: 'Zero-trust remote access.', approach: 'Implement ZTNA with continuous device posture and risk-based access decisions.', example: 'An enterprise uses Zscaler Private Access or Cloudflare Access with device posture checks.', resources: 'Significant — ZTNA platform.' }),
        },
        {
          id: '13.9', title: 'Deploy Port-Level Access Control',
          definition: 'Deploy port-level access control (e.g., 802.1X) on network ports to restrict which devices can connect to enterprise network segments.',
          purpose: 'Prevent unauthorized devices from connecting to the enterprise network.',
          why: 'Open network ports allow rogue devices and supply-chain attacks to gain initial access.',
          ...ig(null, null,
            { scope: '802.1X / NAC enforcement.', approach: 'Deploy 802.1X with certificate-based authentication or MAC bypass with NAC profiling.', example: 'An enterprise uses Cisco ISE or Aruba ClearPass for 802.1X across all wired and wireless ports.', resources: 'Significant — NAC platform + 802.1X infrastructure.' }),
        },
        {
          id: '13.10', title: 'Perform Application Layer Filtering',
          definition: 'Perform application-layer filtering using Layer 7 firewalls, secure web gateways, or web application firewalls to block malicious application traffic.',
          purpose: 'Inspect and block malicious traffic that bypasses lower-layer controls.',
          why: 'Many modern attacks operate within legitimate protocols and require application-aware inspection.',
          ...ig(null,
            { scope: 'Layer-7 filtering at egress.', approach: 'Deploy next-generation firewall or secure web gateway with application-aware policies.', example: 'A mid-sized company uses Palo Alto NGFW with App-ID for egress filtering.', resources: 'Moderate — NGFW.' },
            { scope: 'Enterprise application-layer defense.', approach: 'Deploy combination of NGFW, SWG, CASB, and WAF with consistent policy.', example: 'An enterprise uses Zscaler SWG + Cloudflare WAF + Netskope CASB.', resources: 'Significant — multiple application-layer platforms.' }),
        },
        {
          id: '13.11', title: 'Tune Security Event Alerting Thresholds',
          definition: 'Tune security event alerting thresholds monthly or more frequently to reduce false positives and prioritize high-fidelity alerts.',
          purpose: 'Ensure analysts focus on signal rather than noise.',
          why: 'Alert fatigue causes real incidents to be missed; mature SOCs continuously tune detections.',
          ...ig(null, null,
            { scope: 'Continuous detection tuning.', approach: 'Run monthly alert-quality reviews with detection engineers.', example: 'An enterprise SOC reviews mean-time-to-triage and false-positive rates monthly and tunes rules accordingly.', resources: 'Significant — detection engineering function.' }),
        },
      ],
    },
    ...
    // ════════════════════════════════════════════════════════════════════════════
    {
      ...
      safeguards: [
        ...
        {
          id: '14.8', title: 'Train Workforce on Securing Insecure Networks',
          definition: 'Train workforce members on the risks of connecting to and transmitting enterprise data over insecure networks (public Wi-Fi, untrusted hotspots, personal hotspots).',
          purpose: 'Reduce data leakage and credential theft from remote and travel scenarios.',
          why: 'Insecure networks are a leading vector for man-in-the-middle attacks and session hijacking.',
          ...ig(null,
            { scope: 'Insecure network awareness module.', approach: 'Include a module on Wi-Fi safety, VPN use, and travel security in standard training.', example: 'A mid-sized company adds a dedicated lesson on Wi-Fi risks to onboarding and annual training.', resources: 'Moderate — awareness content.' },
            { scope: 'Travel security program with enforcement.', approach: 'Combine training with enforced always-on VPN and monitoring of risky-network access.', example: 'An enterprise enforces always-on VPN on travel laptops and audits use of public Wi-Fi.', resources: 'Significant — VPN + monitoring stack.' }),
        },
        {
          id: '14.9', title: 'Conduct Role-Specific Security Awareness and Skills Training',
          definition: 'Conduct role-specific security awareness and skills training for high-risk roles such as developers, administrators, finance, legal, and HR personnel.',
          purpose: 'Provide targeted education for roles whose actions disproportionately affect security outcomes.',
          why: 'Generic training under-serves specialised roles whose mistakes carry outsized risk (e.g., wire fraud, code vulnerabilities, account takeover).',
          ...ig(null,
            { scope: 'Targeted training for high-risk roles.', approach: 'Identify high-risk roles and deliver role-specific content at least annually.', example: 'A mid-sized company sends finance staff to BEC training and developers to secure-coding training.', resources: 'Moderate — role-specific content + tracking.' },
            { scope: 'Continuous role-based skills programme.', approach: 'Map roles to skills curricula with periodic skills assessments and certification tracking.', example: 'An enterprise tracks developer secure-coding certifications and SOC analyst threat-hunting skills.', resources: 'Significant — LMS with role mapping.' }),
        },
      ],
    },
    ...
    // ════════════════════════════════════════════════════════════════════════════
    {
      ...
      safeguards: [
        ...
        {
          id: '15.6', title: 'Establish and Maintain a Service Provider Inventory',
          definition: 'Establish and maintain a centralised inventory of service providers, including service description, classification, data accessed, and contract owner.',
          purpose: 'Provide visibility into which third parties touch enterprise data and systems.',
          why: 'You cannot manage third-party risk for providers you do not know exist — shadow SaaS is a major blind spot.',
          ig1: { scope: 'Basic vendor list.', approach: 'Maintain a spreadsheet of key SaaS vendors and data they access.', example: 'A small business tracks its top 20 SaaS tools and their data classification.', resources: 'Minimal — spreadsheet.' },
          ig2: { scope: 'Formal service provider inventory.', approach: 'Maintain inventory in a shared system with categorisation and review cadence.', example: 'A mid-sized company tracks vendors in a TPRM tool with annual review.', resources: 'Moderate — TPRM tooling.' },
          ig3: { scope: 'Continuous SaaS / vendor discovery.', approach: 'Use SaaS management and SSPM tools to continuously discover and inventory third-party services.', example: 'An enterprise uses Productiv or Torii integrated with SSO to discover all SaaS usage.', resources: 'Significant — SaaS management platform.' },
        },
        {
          id: '15.7', title: 'Securely Decommission Service Providers',
          definition: 'Securely decommission service provider relationships, including data return or destruction, access revocation, and contract termination procedures.',
          purpose: 'Prevent residual data exposure and unauthorised access after a vendor relationship ends.',
          why: 'Orphaned vendor access and undeleted data continue to expose the organisation long after a contract ends.',
          ...ig(null,
            { scope: 'Documented offboarding checklist.', approach: 'Maintain a checklist covering data destruction, access revocation, and final attestation.', example: 'A mid-sized company runs a vendor-exit checklist within 30 days of contract end.', resources: 'Moderate — process + legal review.' },
            { scope: 'Automated offboarding workflow.', approach: 'Integrate offboarding into the TPRM platform with automated access removal and certificate of destruction.', example: 'An enterprise uses TPRM workflows to trigger SSO de-provisioning and data deletion attestations.', resources: 'Significant — TPRM + IGA integration.' }),
        },
      ],
    },
    ...
    // ════════════════════════════════════════════════════════════════════════════
    {
      ...
      safeguards: [
        ...
        {
          id: '16.11', title: 'Separate Production and Non-Production Systems',
          definition: 'Maintain separate production and non-production environments with controlled access, sanitized data, and clear promotion procedures.',
          purpose: 'Prevent accidental or malicious changes from non-production reaching production.',
          why: 'Shared environments and live production data in test systems are a major breach and data-leakage vector.',
          ...ig(null,
            { scope: 'Separate environments with controlled promotion.', approach: 'Maintain separate accounts/clusters for prod vs non-prod; restrict prod access; mask data in lower environments.', example: 'A mid-sized company runs prod in a dedicated AWS account with separate IAM and data masking pipelines for staging.', resources: 'Moderate — multi-environment infrastructure.' },
            { scope: 'Strict environment segregation with synthetic data.', approach: 'Use synthetic / fully anonymised data in non-production, segregated networks, and automated promotion gates.', example: 'An enterprise uses synthetic data generators and policy-gated promotion in Spinnaker.', resources: 'Significant — data-generation + governance tooling.' }),
        },
        {
          id: '16.12', title: 'Leverage Vetted Application Security Modules',
          definition: 'Leverage vetted libraries and frameworks (authentication, cryptography, input validation, logging) rather than reimplementing security-critical components.',
          purpose: 'Reduce the risk of vulnerabilities introduced by custom security code.',
          why: 'Hand-rolled crypto, auth, and validation are responsible for many serious application vulnerabilities.',
          ...ig(null,
            { scope: 'Standard libraries for security functions.', approach: 'Mandate use of vetted libraries for crypto, auth, and validation in coding standards.', example: 'A mid-sized company forbids custom crypto and requires use of language-standard libraries.', resources: 'Moderate — coding standard + review.' },
            { scope: 'Curated internal security platform.', approach: 'Provide developers with an approved internal security platform (auth, crypto, secrets) and block alternatives in CI.', example: 'An enterprise provides an internal auth SDK and blocks unapproved crypto libraries via SBOM policy.', resources: 'Significant — platform engineering investment.' }),
        },
        {
          id: '16.13', title: 'Implement Code-Level Security Checks',
          definition: 'Implement code-level security checks including secret scanning, dependency vulnerability scanning, and policy-as-code in pull-request workflows.',
          purpose: 'Catch security defects before they reach the main branch.',
          why: 'Code-level controls are far cheaper than post-deployment remediation and prevent supply-chain leaks.',
          ...ig(null,
            { scope: 'Pre-commit secret + dependency scanning.', approach: 'Enable secret scanning and dependency review on the main repo.', example: 'A mid-sized company enables GitHub Advanced Security secret scanning and Dependabot.', resources: 'Moderate — git provider security features.' },
            { scope: 'Policy-as-code in PR workflow.', approach: 'Block merges on critical findings, enforce SAST/secret/dependency gates in CI.', example: 'An enterprise uses OPA / GitHub Actions to block PRs with critical findings or unapproved licences.', resources: 'Significant — policy-as-code tooling.' }),
        },
        {
          id: '16.14', title: 'Conduct Application Threat Modeling',
          definition: 'Conduct threat modelling for new and significantly changed applications to identify and address security risks early in the lifecycle.',
          purpose: 'Surface design-level security flaws that cannot be caught by code-level scanning.',
          why: 'Architectural flaws drive the most damaging breaches and are nearly impossible to fix late.',
          ...ig(null, null,
            { scope: 'Threat modelling for material changes.', approach: 'Run STRIDE / attack-tree threat modelling sessions on new applications and major refactors.', example: 'An enterprise requires a threat-modelling artefact in the design-review checklist for every new service.', resources: 'Significant — security architects + process discipline.' }),
        },
      ],
    },
    ...
    // ════════════════════════════════════════════════════════════════════════════
    {
      ...
      safeguards: [
        ...
        {
          id: '17.7', title: 'Establish Incident Reporting Channels',
          definition: 'Establish and maintain clear internal and external channels for reporting suspected incidents, including 24/7 contact information, anonymous reporting options, and customer / regulator notification paths.',
          purpose: 'Ensure incidents reach responders quickly through trusted channels.',
          why: 'Delayed or lost reporting extends dwell time and worsens regulatory and reputational impact.',
          ig1: { scope: 'Basic reporting channel.', approach: 'Publish a single email / phone for incident reporting and acknowledge submissions.', example: 'A small business publishes a security@ alias monitored by the IT lead.', resources: 'Minimal — published contact + monitoring.' },
          ig2: { scope: 'Documented reporting paths.', approach: 'Document internal and external reporting channels (employees, customers, regulators), with on-call rotation.', example: 'A mid-sized company runs a security@ alias plus a 24/7 phone tree, with documented regulator contacts.', resources: 'Moderate — on-call rotation + runbooks.' },
          ig3: { scope: 'Multi-channel reporting with intake automation.', approach: 'Provide multiple channels (email, phone, web form, anonymous hotline) integrated with ticketing and SIEM.', example: 'An enterprise uses an integrated intake form that triggers PagerDuty and creates a triage ticket automatically.', resources: 'Significant — intake + automation tooling.' },
        },
        {
          id: '17.8', title: 'Establish Security Incident Severity Thresholds',
          definition: 'Establish and maintain a documented incident severity rating scheme with thresholds that drive escalation, notification, and response intensity.',
          purpose: 'Ensure consistent triage and proportionate response.',
          why: 'Without thresholds, similar incidents get inconsistent responses, leading to under-reaction or burnout.',
          ...ig(null,
            { scope: 'Documented severity matrix.', approach: 'Maintain a severity matrix (e.g., Sev1–Sev4) with criteria and escalation rules.', example: 'A mid-sized company uses a 4-level severity scheme with criteria for data impact, downtime, and visibility.', resources: 'Moderate — written runbook.' },
            { scope: 'Codified severity with automation.', approach: 'Encode severity rules in the SIEM / SOAR so triage tickets are auto-classified and escalated.', example: 'An enterprise uses Sentinel/Splunk SOAR playbooks that auto-classify incidents and page responders by severity.', resources: 'Significant — SOAR investment.' }),
        },
        {
          id: '17.9', title: 'Establish External IR Support Relationships',
          definition: 'Establish and maintain relationships with external IR support — including retained forensics firms, legal counsel, PR / crisis communications, and law-enforcement contacts — before an incident.',
          purpose: 'Eliminate first-incident scramble for external help.',
          why: 'Negotiating retainers and onboarding outside experts mid-incident dramatically extends response time and cost.',
          ...ig(null, null,
            { scope: 'Pre-negotiated external IR network.', approach: 'Maintain on-retainer IR + legal + PR partners with documented activation procedures.', example: 'An enterprise retains a forensics firm, breach counsel, and crisis-PR firm with documented activation contacts.', resources: 'Significant — retainer + legal investment.' }),
        },
      ],
    },
