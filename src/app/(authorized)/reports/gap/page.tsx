...
export default function GapReportPage() {
  ...
  const navigationSequence = useMemo(() => {
    ...
    for (const control of CIS_CONTROLS) {
      // Add control (even if inactive, for navigation purposes)
      items.push({ type: 'control', controlId: control.id });

      // Add active safeguards under this control
  ...
  const finalizeReport = async (remarks: string) => {
    ...
    // Collect CMMI data for all active safeguards
  ...
  // Don't show error for summary page
