...
import { ChevronLeft, ChevronRight, X, Check, Search, Building2, Loader2, CheckCircle2 } from 'lucide-react';
...
function OrganizationOnboardContent() {
  ...
  // Get organizationId from URL if editing existing org
  ...
  const handleCompanySelect = useCallback((company: CompanySearchResult) => {
    ...
    const applyIfPresent = (key: keyof FormData, value: string | null) => {
      if (value !== null) {
        newAutoFilled[key] = { source: company.source, confirmedAt: null };
      }
    };
  ...
  const saveData = useCallback(
    async (data: FormData) => {
      ...
      try {
        ...
        if (!isEditing && result.success && result.data?.id) {
          // Update URL to include the new organization ID for subsequent saves
          router.replace(`/organization/onboard?id=${result.data.id}`);
        }
    ...
    [isEditing, organizationId, router, t, autoFilledFields]
  );
  ...
  // Handle exit
  const handleExit = () => {
    router.push(returnUrl || '/organization');
  };
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <Button variant="secondary" size="sm" onClick={handleExit} className="gap-2">
              <X className="w-4 h-4" />
              {tc('navigation.exit')}
            </Button>
          </div>
          ...
          <div className="border rounded-lg p-6 space-y-6">
            ...
            {/* Company lookup — shown only on basics step, only when countries are configured */}
